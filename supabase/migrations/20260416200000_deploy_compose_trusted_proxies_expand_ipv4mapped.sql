-- Expand gateway.trustedProxies with ::ffff:<ipv4> entries (Node often reports IPv4-mapped IPv6 remotes),
-- so "Proxy headers detected from untrusted address" clears when OPENCLAW_GATEWAY_TRUSTED_PROXIES lists bare IPv4.
    ...
    command:
      ...
      - |
        ...
        if (Array.isArray(out.gateway.trustedProxies) && out.gateway.trustedProxies.length) {
          out.gateway.trustedProxies = expandTrustAddrs(out.gateway.trustedProxies);
        }
        out.agents = {
          ...agIn,
          defaults: {
        ...
        console.error(JSON.stringify({ sessionId: '225c6f', hypothesisId: 'H-merge', location: 'openclaw-workspace-init', message: 'merged openclaw.json', data: { hadFile, parseOk, topKeys: Object.keys(out), hasEnvBlock: !!out.env && Object.keys(out.env).length > 0, model: M, trustedSet: !!(trustedFromEnv && trustedFromEnv.length), trustedExpandedLen: Array.isArray(out.gateway.trustedProxies) ? out.gateway.trustedProxies.length : 0 }, timestamp: Date.now() }));
