-- openclaw-workspace-init: optional OPENCLAW_BASE_JSON_B64 (from Deploy bot UI via n8n) merged under on-disk openclaw.json; gateway.auth token, trusted proxies, etc.
-- Deploy agent seeds openclaw.json if missing (see deploy-agent/README.md). Paste compose from docs or apply migrations.
