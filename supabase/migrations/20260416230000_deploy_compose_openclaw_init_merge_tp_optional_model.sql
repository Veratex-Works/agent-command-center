-- openclaw-workspace-init: merge trustedProxies from file + env (union); only patch agents.defaults.model
-- when OPENROUTER_MODEL is non-empty so bind-mounted openclaw.json keeps bot/model config otherwise.
